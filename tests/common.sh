...
shared_tests() {
    ...
    # warmup. Retry up to ~3 min. Wrapped in a subshell so `exit` stays
    # local — `check` uses `eval` in the current shell.
    check "health endpoint 2xx on :2971" \
        "(for i in {1..36}; do curl -sfo /dev/null http://localhost:2971/ && exit 0; sleep 5; done; exit 1)"
