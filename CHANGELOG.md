...
- `MARIMO_CACHE_DIR` environment variable for relocating both the `uv` sandbox cache and the Hugging Face hub cache. Caches default to `/home/runpod/.cache` (ephemeral container storage, unchanged from prior releases); set `MARIMO_CACHE_DIR=/workspace/.cache` to persist them on a Runpod network volume so downloaded notebook dependencies and model files survive pod rebuilds. `UV_CACHE_DIR` and `HF_HOME` can still be set individually if fine-grained control is needed.
