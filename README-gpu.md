...
A Docker image that runs [marimo](https://marimo.io) as a notebook server on Runpod GPU pods, served on port **2971** via Runpod's web proxy.
A CPU variant is published with a `-cpu` tag suffix (e.g., `0.9.0-cpu`).
Full docs: [github.com/briandconnelly/runpod-marimo](https://github.com/briandconnelly/runpod-marimo)

## Host requirements

Requires an NVIDIA GPU host with a driver supporting CUDA 12.5 or newer (Linux driver >= 555.42.06) — on an older driver the container refuses to start with `unsatisfied condition: cuda>=12.5`.
...
No domain packages are pre-installed — neither marimo's `recommended` extras (pandas, polars, matplotlib) nor GPU packages (PyTorch, JAX, CuPy).
...
- **CUDA runtime** — built on `nvidia/cuda:*-runtime-ubuntu24.04`, so `nvidia-smi` and the CUDA libraries are available out of the box
