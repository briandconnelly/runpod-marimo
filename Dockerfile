...
# match what start_marimo.sh uses — including the `--with 'mcp<2'` cap,
# which is part of the requirement set uvx keys the cache on. See the
# comment above that invocation for why the cap exists and when to drop it.
# Users who override MARIMO_VERSION at runtime pay the download cost once
